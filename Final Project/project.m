...
% vert concatenates all variables into an input matrix
...
for a=1:1000
    ...
    for h=1:num_columns
        ...
        % Total max/min temperatures and precip
...
for j = 1:num_columns
    total = input(1,j) + input(2,j) + input(3,j);
    if total > 0     
        output_matrix(1,j) = 1;
    elseif total < 0
        output_matrix(1,j) = 0;
